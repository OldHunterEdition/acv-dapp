  ...
  async function verify() {
    ...
    try {
      if (!ethers.isHexString(certId, 32)) {
        throw new Error(
          "certId must be a 32-byte hex string (0x... length 66)."
        );
      }
      ...
      if (!cid)
        throw new Error(
          "No record found for this certId (did you issue it on this chain?)."
        );
      ...
      if (!res.ok)
        throw new Error(`Failed to fetch PDF from IPFS (${res.status})`);
      ...
      result =
        localHash.toLowerCase() === onchainHash.toLowerCase()
          ? "✅ VALID: on-chain hash matches the IPFS PDF"
...
>
  ...
  {#if details}
    <pre
      style="white-space:pre-wrap; background:#0f172a15; padding:0.75rem; border-radius:8px;">{details}</pre>
  {/if}
  ...
  {#if pdfUrl}
    <p>
      ...
      <a href={metadataUrl} target="_blank" rel="noreferrer"
        >Open metadata.json</a
      >
    </p>
  {/if}

  <p style="margin-top:1rem; font-size:0.9rem;">
    Tip: append <code>?contract=0x...&certId=0x...&chainId=11155111</code> to the
    URL to prefill.
  </p>
</main>
