...
contract CertRegistry is AccessControl {
    ...
    event Issued(bytes32 indexed certId, address indexed issuer, string cid, bytes32 contentHash);
    event Revoked(bytes32 indexed certId, address indexed issuer, string reason);
    ...
    function issue(bytes32 certId, string calldata cid, bytes32 contentHash) external onlyRole(ISSUER_ROLE) {
        require(certs[certId].status == Status.None, "exists");
        certs[certId] = Cert(msg.sender, cid, contentHash, uint64(block.timestamp), 0, Status.Active);
    ...
    function revoke(bytes32 certId, string calldata reason) external {
        ...
        require(hasRole(ISSUER_ROLE, msg.sender) && msg.sender == c.issuer, "only issuer");
